...
//		bitmapUtils.configDefaultLoadingImage(R.drawable.pic_default);// 默认背景图片
//		bitmapUtils.configDefaultLoadFailedImage(R.drawable.pic_default);// 加载失败图片
	...
	private void fadeInDisplay(ImageView imageView, Bitmap bitmap) {// 目前流行的渐变效果
		...
		int screenWith = LightApplication.getInstance().getWithPx();
		float scale = (float)bmpWidth / (float)bmpHeight;
		float scaleWidth = screenWith/bmpWidth;
		float scaleHeight = ScreenUtils.dpToPx(LightApplication.getInstance(), 250)/bmpHeight;
...
//		}
		...
		final TransitionDrawable transitionDrawable = new TransitionDrawable(
				new Drawable[] { TRANSPARENT_DRAWABLE,
						new BitmapDrawable(imageView.getResources(), resizeBmp) });
